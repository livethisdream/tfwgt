function y = SelfIntegral_5(w,et,ez,mut,muz,wm,wn,vm,vn,a,b,d,Mhxm,Mhxn,Mhym,Mhyn,lamx)
...
Aterm2=( (4.*pi.*w.*ez.*eps0.*Clamx)./(d) ).*...
    ...
    ( (lamylpsi.^2 + lamxl.^2).*(lamylpsi.^2 - kym.^2).*(lamylpsi.^2 - kyn.^2).*( 1+1.*(l==0) ) )...
        ,1);
...
Cterm2= (  (4.*pi.*w.*ez.*eps0.*lamx.^2.*Clamx)./(d) ).*...
		sum( ...
			(  (lamylpsi).*( 1-exp(-1j.*lamylpsi.*b) )  )./...
    			(  (lamylpsi.^2 + lamxl.^2).*(lamylpsi.^2 - kym.^2).*(lamylpsi.^2 - kyn.^2).*( 1+1.*(l==0) )  ),1 ...
		    );
...
Dterm2 =  (  (4.*pi.*w.*ez.*eps0.*lamx.^4.*Clamx)./(d) ).*...
		sum( ...
			(  ( 1-exp(-1j.*lamylpsi.*b) )  )./...
    			(  (lamylpsi).*(lamylpsi.^2 + lamxl.^2).*(lamylpsi.^2 - kym.^2).*(lamylpsi.^2 - kyn.^2).*( 1+1.*(l==0) )  )...
            ,1);
