function y = SelfIntegral_3(w,et,ez,mut,muz,wn,vm,vn,a,b,d,Mhxm,Mhxn,Mhyn,lamx)
...
A11sumterm1= ( (mu0*muz.*lamx.^2.*Clamx)./(w*mu0^2*mut^2)).*...
    sum( ( ( (pi*l/d).^2 ).*(1-exp(-1j*lamylth*b) ) )./( lamylth.*(lamylth.^2 + lamxl.^2).*(lamylth.^2 - kyn^2) ),1);


% % tmz sum term
lamylpsi=sqrt(kzt^2)*sqrt(1-((pi^2*l.^2)/(d^2*kt^2))-((lamxl.^2)/(kzt^2)));
A11sumterm2=(w.*ez.*eps0.*Clamx).*sum( ( lamylpsi.*(1-exp(-1j.*lamylpsi.*b) ) )./...
    ( (lamylpsi.^2 + lamxl.^2).*(lamylpsi.^2 - kyn^2).*(1+1.*(l==0)) ),1);
...
% the tez sum term
B11sumterm1= -( (mu0*muz.*Clamx)./(w*mu0^2*mut^2) ).*...
    sum( ( ( (pi*l/d).^2 ).*(1-exp(-1j*lamylth*b) ) )./( lamylth.*(lamylth.^2 + lamxl.^2).*(lamylth.^2 - kyn^2) ),1);

% % tmz sum term
B11sumterm2=(w.*ez.*eps0.*Clamx).*sum( (1-exp(-1j.*lamylpsi.*b) )./...
    ( lamylpsi.*(lamylpsi.^2 + lamxl.^2).*(lamylpsi.^2 - kyn^2).*(1+1.*(l==0)) ),1);
