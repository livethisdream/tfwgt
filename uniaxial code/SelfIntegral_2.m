function y = SelfIntegral_2(w,et,ez,mut,muz,wm,vm,vn,a,b,d,Mhxm,Mhxn,Mhym,lamx)
...
A11sumterm1= ((mu0*muz.*lamx.^2.*Clamx)./(w*mu0^2*mut^2)).*...
    sum( ( ( (pi*l/d).^2 ).*(1-exp(-1j*lamylth*b) ) )./( lamylth.*(lamylth.^2 + lamxl.^2).*(lamylth.^2 - kym^2) ),1);


% % tmz sum term
lamylpsi=sqrt(kzt^2)*sqrt(1-((pi^2*l.^2)/(d^2*kt^2))-((lamxl.^2)/(kzt^2)));
A11sumterm2=(w.*ez.*eps0.*Clamx).*sum( ( lamylpsi.*(1-exp(-1j.*lamylpsi.*b) ) )./...
    ( (lamylpsi.^2 + lamxl.^2).*(lamylpsi.^2 - kym^2).*(1+1.*(l==0)) ),1);
...
B11sumterm2=(w.*ez.*eps0.*Clamx).*sum( (1-exp(-1j.*lamylpsi.*b) )./...
    ( lamylpsi.*(lamylpsi.^2 + lamxl.^2).*(lamylpsi.^2 - kym^2).*(1+1.*(l==0)) ),1);

% put it all together
B11lamy= ( (4*pi*Mhym*Mhxn*wm*vn.*lamx.^2)./ (a*b*d) ).*(B11sumterm1+B11sumterm2);
