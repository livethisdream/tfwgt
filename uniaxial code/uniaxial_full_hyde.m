...
global widx m_mat n_mat a b dmat eps0 mu0 numModes includeModes q_mat...
    solveCase porttouse alg ket kmut num_int; 

% % % % % % % % % % % % % % % % % % % %
% % constants & global parameters
% % % % % % % % % % % % % % % % % % % %

% m_mat=[1 3 1 5 3 5 7 7 9 1 3 9 5]; % the first 20 modes
% n_mat=[0 0 2 0 2 2 0 2 0 4 4 2 4];
m_mat=[1 3 1 1 1 1];
n_mat=[0 0 2 4 6 8];
c=3.0e8;           
eps0=8.854e-12;
...
vers='v1'; % the version of the measurement2.Or are
orientation='_O2'; % needs the preceeding '_'
methd='tfwgt'; % tfwgt or nrw
add2casedesc='_hyde_debug';  % a string to add to the case description (put leading underscore)
porttouse=3; % 1=use S11 & S21, 2=use S22 & S12, 3=use all
includeModes=[1 2]; % which mode (indices) do you want to include?
q_mat=includeModes;
self_cal=0; % 1=provide your own TRL cal files, 0=use VNA cal'd data
errchk=0;    % error bar routine? 1=yes, 0=no

% % solver options 
numds=5;  % downsample the data to this number of points, 0 equates to no downsampling
init_method='nrwTruth'; % 'nrw','nrwTruth','initone', or 'initup'
...
makeFile=0;  % output the results to a csv (*.dat) file? 1=yes, 0=no
print=0;
smooth_zvals=0;
smooth_tvals=0;
smeth='sgolay';
...
if strcmp(init_method,'nrw')==1
  ...
  % NRW analysis
    ...
    [epsfwd,epsbw,mufwd,mubw]=waveguide_nrw(fnrw,realS11ex,imagS11ex,realS21ex,imagS21ex,...
        realS12ex,imagS12ex,realS22ex,imagS22ex,dmat(1));
    epsNRW=epsbw;
    muNRW=mubw;
...
% % % % % % % % % % % % % % % % % % % % %  
    ...
    [etsol(widx), ezsol(widx), mutsol(widx), muzsol(widx)] ...
        = runSolver_hyde(Smeas,wval,etguess(widx),ezguess(widx),mutguess(widx),muzguess(widx));
...
% % % % % % % % % % % % % % % % % % % % %  
    ...
    if errchk==1
           [stddeltaetreal(widx), stddeltaetimag(widx), stddeltaezreal(widx), stddeltaezimag(widx),...
               ...
               = getErrorTerms_hyde(etsol(widx),ezsol(widx),mutsol(widx),muzsol(widx),Smeas,...
