function [etsol,ezsol,mutsol,muzsol] = runSolver_hyde(Smeas,wval,etguess,ezguess,mutguess,muzguess)
...
switch solveCase
    case 1 % isotropic, dielectric, non-mag -> ez=et and muz=mut=mu0
        if strcmp(alg,'LM')==1
            Y=lsqcurvefit(@Sparams_hyde,[real(etguess) imag(etguess)],...
                wval,Smeas,[],[],options);
        elseif strcmp(alg,'TRR')==1 
    ...
    case 2 % isotropic, non-dielectric, mag -> ez=et=eps0 and muz=mut
        if strcmp(alg,'LM')==1
            Y=lsqcurvefit(@Sparams_hyde,[real(mutguess) imag(mutguess)],...
                wval,Smeas,[],[],options);
        elseif strcmp(alg,'TRR')==1 
            Y=lsqcurvefit(@Sparams_hyde,[real(mutguess) imag(mutguess)],...
                wval,Smeas,[0 lb],[ub 0],options);
...
%                 wval,Smeas,[lb lb lb lb],[ub ub ub ub],options);
    ...
    case 7 % uniaxial, dielectric, mag with known et and mut
        if strcmp(alg,'LM')==1
            Y=lsqcurvefit(@Sparams_hyde,[real(ezguess) imag(ezguess) real(muzguess) imag(muzguess)],...
                wval,Smeas,[],[],options);
        elseif strcmp(alg,'TRR')==1
            Y=lsqcurvefit(@Sparams_hyde,[real(ezguess) imag(ezguess) real(muzguess) imag(muzguess)],...
                wval,Smeas,[0 lb 0 lb],[ub 0 ub 0],options);
