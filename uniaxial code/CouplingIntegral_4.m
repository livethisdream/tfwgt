function y = CouplingIntegral_4(w,et,ez,mut,muz,wm,vm,vn,a,b,d,Mhxm,Mhxn,Mhym,Mhyn,lamx)
...
Aterm2unstable=(  1 )./( sin(lamztha*d) ) ;
...
% % tmz terms
% the sum term 
Aterm3=-( (4.*pi.*w.*ez.*eps0.*Clamx)./(d) ).*...
    sum( ... 
    ( (lamylpsi.^3).*( 1-exp(-1j.*lamylpsi.*b) ) )./...
    ( (lamylpsi.^2 + lamxl.^2).*(lamylpsi.^2 - kya.^2).^2.*( (-1).^(l) + 1.*(l==0) ) )...
        ,1);

% the fourth term
Aterm4stable=(  (1j*pi*b.*w.*et.*eps0.*kya.^2.*Clamx)./( (lamx.^2 + kya.^2) )  );
Aterm4unstable=( 1 )./( lamzpsia.*sin(lamzpsia*d) );
...
Bterm2unstable=( 1 )./( sin(lamztha*d) ) ;
...
Bterm4unstable= ( 1 )./( lamzpsia.*sin(lamzpsia*d) );
...
Cterm2unstable=( 1 )./( sin(lamztha*d) ) ;
...
Cterm4unstable= ( 1 )./( lamzpsia.*sin(lamzpsia*d) );
...
Dterm2unstable=( 1 )./( sin(lamztha*d) ) ;
...
Dterm3= - (  (4.*pi.*w.*ez.*eps0.*lamx.^4.*Clamx)./(d) ).*...
		sum( ...
			(  ( 1-exp(-1j.*lamylpsi.*b) )  )./...
    		(  (lamylpsi).*(lamylpsi.^2 + lamxl.^2).*(lamylpsi.^2 - kya.^2).^2.*( (-1).^(l) + 1.*(l==0) )  )...
            ,1);

% the fourth term
Dterm4stable= ( 1j*pi*b.*w.*et.*eps0.*lamx.^4.*Clamx)./( kya.^2.*(lamx.^2 + kya.^2) ) ;
Dterm4unstable= ( 1 )./( lamzpsia.*sin(lamzpsia*d) );
